...
const DialogBox = ({ isOpen, onClose, setIsOpen,id ,isBookmark}) => {
    console.log(id , ": ",isBookmark);
  const [dialogContent, setDialogContent] = useState();

  const [saved, setSaved] = useState(isBookmark);
 ...
 const api_url = `https://api.nal.usda.gov/fdc/v1/food/${id}?api_key=rtvxHn8xJkOelav5gz5Trjd2RuhW7n4FwI5CPvAs`
  ...
  return (
    <div className={`${isOpen ? "fixed inset-0 overflow-y-auto" : "hidden"} z-10`}>
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left  transform transition-all " style={{height:"100vh"}}>
...
<div>
          <div className="bg-gray-50 flex flex-col overflow-y-auto" style={{height:"80vh"}}>
            ...
            <div className="flex justify-end relative bottom-6 right-10">
              <button
                onClick={handleClose}
                type="button"
